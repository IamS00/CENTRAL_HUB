...
export default function EditMaterialPage() {
  ...
  const fetchMaterial = async () => {
    try {
      const response = await fetch(`/api/resources/${materialId}`)
      const data = await response.json()

      if (data.success) {
        const material = data.data
        setFormData({
          titleRo: material.titleRo,
          titleEn: material.titleEn || '',
          descriptionRo: material.descriptionRo || '',
          descriptionEn: material.descriptionEn || '',
          type: material.type,
          fileUrl: material.fileUrl,
          categoryIds: material.categories.map((cat: Category) => cat.id),
        })
      } else {
      ...
      setError('Eroare la încărcarea materialului')
    } finally {
  ...
  if (loadingData) {
    return (
      <div className="flex items-center justify-center h-full">
        <div className="text-gray-500">Se încarcă materialul...</div>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Editează Material</h1>
          <p className="text-gray-600 mt-1">Modifică detaliile materialului</p>
        </div>
      </div>
